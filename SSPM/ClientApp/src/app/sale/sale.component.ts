import { MessageService, ConfirmationService, OverlayPanel } from 'primeng/primeng';
import { Component, OnInit } from '@angular/core';
import { Product } from '../models/product';
import { ProductService } from '../services/product.service';
import { TransactionService } from '../services/transaction.service';
import { TransactedProduct } from './../models/transacted-product';
import { Transaction, DiscountType } from '../models/transaction';
...
export class SaleComponent implements OnInit {
  ...
  newSale: Transaction = new Transaction();
  ...
  ngOnInit() {
    ...
    var percentage = new SelectedItem()
    percentage.label = 'Perqindje (%)';
    percentage.value = 1;

    var euro = new SelectedItem()
    euro.label = 'Euro (€)';
    euro.value = 2;
  ...
  onRemoveFromList(index) {
    this.newSale.initialPrice = this.newSale.initialPrice - (this.newSale.transactedProducts[index].transactionPrice * this.newSale.transactedProducts[index].transactionQuantity);
    this.addDiscount()
    this.newSale.transactedProducts.splice(index, 1);
  }
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        if (this.newSale.discountType == DiscountType.percentage) {
          this.newSale.transactedProducts.forEach(element => {

            element.transactionPrice = element.transactionPrice - (element.transactionPrice * 0.01 * this.newSale.discount);

          });
        }
        if (this.newSale.discountType == DiscountType.euro) {
          this.newSale.transactedProducts.forEach(element => {

            var euroPercentage = this.newSale.discount * 100 / this.newSale.initialPrice;

            element.transactionPrice = element.transactionPrice - (element.transactionPrice * 0.01 * euroPercentage);
          });
        }

        this.newSale.totalQuantity = 0;

        this.newSale.transactedProducts.forEach(element => {
          this.newSale.totalQuantity = this.newSale.totalQuantity + element.transactionQuantity;
        });


        this.transactionService.addSale(this.newSale).subscribe((res: Transaction) => {
          this.newSale = res;
  ...
  addDiscount() {
    ...
    if (this.newSale.discountType == DiscountType.percentage) {
      if (this.newSale.discount > 100) {
        this.newSale.discount = 100;
      }
      this.newSale.totalPrice = this.newSale.initialPrice - (this.newSale.initialPrice * 0.01 * this.newSale.discount);

    }
    else if (this.newSale.discountType == DiscountType.euro) {
      if (this.newSale.discount > this.newSale.initialPrice) {
        this.newSale.discount = this.newSale.initialPrice;
      }
      this.newSale.totalPrice = this.newSale.initialPrice - this.newSale.discount;
  ...
  transactionPrint(transaction: Transaction) {
    ...
    this.transactionService.getTransactedProductsByTransactionIdNoFilter(transaction.id).subscribe((res: PagedList) => {
      ...
      var title = 'Fature e shitjes - Autodekor Skenda';
      ...
      let dd = {
        ...
        content: [
          ...
          {
            columns: [
              { text: 'Nr. tel: +383 49 536 396', alignment: 'left', fontSize: 10 },
